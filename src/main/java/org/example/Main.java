...
public class Main {
    public static void main(String[] args) throws Exception {
        OntologyLoader.loadOntology("Facebook", new File("Facebook.txt").toPath());
        OntologyLoader.loadOntology("Twitter", new File("Twitter.txt").toPath());
        ...
        try (var embedding = EmbeddingFactory.getSimilarityScore(EmbeddingCases.ConceptNetWithMatch)) {
            var Aligner = new OntologyAlignment(OntologyLoader.getLoadedOntology("Facebook"),
                    OntologyLoader.getLoadedOntology("Twitter"),
                    embedding); // Initialising the alignment function
            var A = Aligner.run(true, true); // Running the alignment
            Aligner.exportGraph(new File("graph.dot"));
            System.out.println(A); // A printing aid, showing what the alignment should represent, the correspondence across fields within the class
        } catch (Exception e) {
